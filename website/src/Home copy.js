...
export default function Home({ searchValue }) {
  ...
  const handleProgress = (state, ele) => {
    ...
    const video = videoRefs.current[ele?.contentId]?.current;
  ...
  const fetchData = async () => {
    ...
    let isReccomend = await listOfMovie?.filter(
      (ele) => ele.isrecommended === true
    );

    // let ContinewWatchingdata = listOfMovie
    //   ?.filter((movie) => {
    //     return ContinewWatching?.continueWatching?.some((cont) =>
    //       movie?._id?.includes(cont.contentId)
    //     );
    //   })
    //   ?.map((movie) => {
    //     const matchingCont = ContinewWatching?.continueWatching?.find(
    ...
    //     return movie;
    //   });
    let ContinewWatchingEpisode = Episodes?.filter((episode) => {
      return ContinewWatching?.continueWatching?.some((cont) =>
    ...
    //   ContinewWatchingdata,
    //   searchValue
    // );
    const filteredSeriesda = filterDataBySearchValue(Seriesda, searchValue);
    const filteredlistOfMovie = filterDataBySearchValue(
      listOfMovie,
      searchValue
    );
    const filteredmostvieved = filterDataBySearchValue(mostvieved, searchValue);
    const filteredisReccomend = filterDataBySearchValue(
      isReccomend,
      searchValue
    );
    ...
    setContinewWatching(ContinewWatching.continueWatching);
    setSeriesData(filteredSeriesda);
    setContentData(filteredlistOfMovie);
    setMostViewedM(filteredmostvieved);
    setTrendingMovies(filteredTrendingdata);
    setIsRecomded(filteredisReccomend);
    setBannerData(BannerData);
    setTrendingMoviesSlider(Trendingdata);
    setWishlistContent(Wishlist);
    setpurchaceContentData(listOfMovie);
    setMergedData(listOfMovie.concat(Episodes));
  };

  const WatchContentMode = async (Item) => {
    try {
  ...
  const incrementViews = async (Item) => {
    try {
      let FilredData = await MergedData.find(
        (ele) =>
          ele._id === Item?.contentId || ele?.contentId === Item?.contentId
      );
      let SeriseData = await ContentData.find(
        (ele) => ele._id === FilredData?.series_id
      );

      if (ContentData.length > 0 && Item.section === "movie") {
        await ContentsPageService.postViews(Item?.contentId);
        navigate("/Playlist", { state: { Item: FilredData, type: "movie" } });
      } else {
        await ContentsPageService.postViews(Item.contentId);
        navigate("/EPlaylist", {
          state: {
      ...
      <div className="row m-auto bg-mg">
            ...
            {BannerData?.map((Ele, index) => {
              ...
              return (
                <Carousel.Item key={index} className="relativeP">
                  ...
                  <Link state={{ id: Ele?._id }} to="/viewdetails-about-movie">
                  ...
                  >
                    <div className="row mb-3">
                      <img
                        src={BannerData[currentSlide]?.titleImg}
                        alt=""
                        className="col-md-6"
                        height={100}
                        width={200}
                        ...
                        <div className="row ">
                          <span className="col-md-3  m-auto  text-white ">
                            <PlayCircleFilledWhiteIcon
                              onClick={() =>
                                WatchContentMode(BannerData[currentSlide]?._id)
                              }
                              className="playicon"
                            />{" "}
                          </span>
                              ...
                              <span className="col-md-6 m-auto text-white detailsList relativeP ">
                                ...
                                <button className="col-md-3  details">
                                  <Link
                                    className="text-dark"
                                    state={{
                                      id: BannerData[currentSlide]?._id,
                                    }}
                                  >
                                    Details
                                  </Link>
                ...
                <div className="mediaScreen">
                  <div className="banner ">
                    {ContinewWatching?.slice(
                      ...
                      const isWishlist = WishlistContent?.some(
                        (wishl) => wishl?.content_id === item.contentId
                      );
                        ...
                        <div key={item._id} className="">
                          <div className="mediaDiv ">
                            <div className="relativeP">
                              <h1 className="text-white">{item.section}</h1>
                              <img
                                src={item.banner}
                                alt={item.name}
                                className="mediaImg"
                            ...
                            <div className="displayhoverScreen">
                              <div className="hoverScreen">
                                <div className="mediaHoverImg">
                                  {hoveredVideoId === item?.contentId &&
                                    hideimage && (
                                      <>
                                        <ReactPlayer
                                          ref={(ref) =>
                                            (videoRefs.current[item.contentId] =
                                              ref)
                                          }
                                          controls="0"
                                          muted={!Volume}
                                          ...
                                          type="video/mp4"
                                          onReady={() =>
                                            handleReady(item.contentId, item)
                                          }
                                          url={item?.video}
                                          playing={isplaying}
                                ...
                                <div className="hoverData">
                                  ...
                                  <div className="row  ">
                                    <div className="col-md-6 m-auto text_White ">
                                      <div className="row">
                                        ...
                                        <span
                                          ...
                                          onMouseMove={(e) =>
                                            handleMouseMove(e, item?.contentId)
                                          }
                                          onMouseOver={() =>
                                            handleMouseOver(
                                              item.contentId,
                                    ...
                                    <div className="col-md-3 text_White m-auto relativeP slidewatch">
                                      {isWishlist ? (
                                        <CheckIcon
                                          className="addicon"
                                          onClick={() =>
                                            handleAddToWishlist(item.contentId)
                                          }
                                        />
                                      ) : (
                                        <AddIcon
                                    ...
                                    <div className="col-md-3 text_White m-auto relativeP viewmore">
                                      <MoreVertIcon
                                        onClick={() =>
                                          handlePurchaceContent(item?.contentId)
                                        }
                                        className="addicon"
                                      />
        ...
        <div className="row mt-3">
          {mostViewedM?.length > 0 && (
            <>
              ...
              <CaroselComponent
                data={mostViewedM}
                incrementViews={WatchContentMode}
            ...
            {visibleCards?.length > 6 && (
              <>
                ...
                <button
                  ...
                  className={`slidebtn2 text-white ${
                    (currentSlides === SeriesData.length) === 0
                      ? "disabled"
                      : ""
                  }`}
                >
                  ...
                  <div key={index} className="cardss">
                    ...
                    >
                      ...
                      <div className="row ">
                        <span className="col-md-3  m-auto  text-white ">
                          <PlayCircleFilledWhiteIcon
                            onClick={() => WatchContentMode(image?._id)}
                            className="playicon"
                          />{" "}
                        </span>
                        <span className="col-md-4 m-auto  text-white">
        ...
        {TrendingMovies?.length > 0 && (
          <div className="row m-auto  relativeP">
                        ...
                        <div className="col-md-7">
                          <div className="row ">
                            <span className="col-md-2  m-auto  text-white ">
                              <PlayCircleFilledWhiteIcon
                                onClick={() => WatchContentMode(Ele?._id)}
                                className="playicon"
                              />{" "}
                            </span>
            ...
            <div className="row " style={{ position: "absolute", top: "70%" }}>
              <MultiCarousel responsive={responsive}>
                {TrendingMoviesSlider?.map((ele, inneindex) => (
                  <>
                    <div key={ele?.id} className="col-md-10 m-auto    ">
                      <div className="row">
