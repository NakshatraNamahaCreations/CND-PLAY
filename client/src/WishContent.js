...
export default function WishListCon() {
  ...
  const handelprevSlider = (index) => {
    if (index === 0) {
      setCurrentSlide((prevSlide) =>
        prevSlide > 0 ? prevSlide - 1 : WishlistContent.length - 5
      );
    }
  };
  ...
  const handelnextSlider = (index) => {
    if (index === 0) {
      setCurrentSlide(
        (prevSlide) => (prevSlide + 1) % (WishlistContent.length - 5)
      );
    }
  };
  ...
  return (
    <div className="col-md-12  bg-mg">
      ...
      <div className="row m-auto   relativeP">
        ...
        {WishlistContent?.slice(currentSlide, currentSlide + 4)?.map((ele) => {
          let { banner, title, duration, storyline } = ele;
          return (
            <div key={ele?.id} className="col-md-3 m-auto  movie-container  ">
              ...
              <div className="row  m-auto additional-content ">
                ...
                <p className="fnt12 textbold">{title}</p>
                <p className="fnt12 ">{duration}</p>
                <p className="fnt12 ">
                  {title}-{storyline}
                </p>
              </div>
