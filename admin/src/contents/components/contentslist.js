...
import { retrieveContents, changeContentsStatus } from "../actions/contents";
...
const ContentsList = () => {
  ...
  const fetchData = async () => {
    try {
      let ContentData = await ContentsPageService.fetchContentsList(
        dataCount,
        page
      );
  ...
  const handleChangeStatusContentsRecord = (index, status) => {
    ContentsPageService.changeContentsStatus({
      ...
      .then((data) => {
        ...
        ContentsPageService.fetchContentsList({
          datacount: dataCount,
          page: page,
        });
      })
      ...
      if (ele._id === id && child.current && ele._id) {
        ...
        const updatedData = {
          ...
          data: {
            ...
            title: title,
            section: section,
            bgcolor: bgcolor?.startsWith("#") ? bgcolor?.substring(1) : bgcolor,
            type: type,
            amount: amount,
            validity: validity,
            storyline: storyline,
