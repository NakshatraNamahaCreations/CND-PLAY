...
import ContentsCreate from "../../contents/components/contentscreate";
const SeriesList = () => {
  const dataCount = 5;
  ...
  const fetchData = async () => {
    try {
      let UpcomingData = await ContentsPageService.fetchSeriesList(
        dataCount,
        page
      );
  ...
  const handlePageNext = async () => {
    try {
      ...
      console.error("Error fetching content Movie list:", error);
    }
  };
  ...
  const handleChangeStatusUpcomingRecord = (index, status) => {
    ContentsPageService.changeContentsStatus({
      ...
      .then((data) => {
        ...
        ContentsPageService.fetchSeriesList({
          datacount: dataCount,
          page: page,
        });
